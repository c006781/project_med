...
class IEditableComponent(ABC):
    ...
    def set_draft_change_notifier(self, notifier: Callable[[int, int], None]) -> None:
        ...
        **ВАЖНО:** 
            Дочерний компонент НЕ ДОЛЖЕН вызывать этот callback самостоятельно.
            Весь учёт изменений количества черновиков выполняет родительская страница (PaginatedListPage).
            Если компонент всё же вызовет notifier, это приведёт к двойному учёту и дисбалансу счётчиков.
            
            Данный метод существует только для того, чтобы родительская страница могла передать свой callback,
            который она будет вызывать в нужные моменты (например, после применения черновика).
