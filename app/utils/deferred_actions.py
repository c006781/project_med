...
def execute_actions(
    ...
    ВАЖНО: из-за асинхронного вызова через QTimer.singleShot необходимо фиксировать
    аргументы замыкания с помощью параметров по умолчанию (см. реализацию), иначе
    все вызовы получат последние значения из цикла.
