    ...
    **Работа с множественными дочерними компонентами разных типов:**
    ...
    def _del_file(
        ...
        **Особенности:**
        ...
        # Добавляем ранний выход при None
        if file_path is None:
            self.logger.debug("_del_file вызван с None, удаление не требуется")
            return None
    ...
    def _is_file_in_temp_dir(
        ...
        **Предположение:**
        ...
        if not temp_dir or not value:
            return False
        #
        # # Если value уже является абсолютным путём, проверяем, лежит ли он внутри temp_dir
        # if os.path.isabs(value):
        #     return os.path.commonpath([value, temp_dir]) == temp_dir
    ...
    def _ensure_temp_dir(self, entity_id: int) -> str:
        ...
        if not temp_dir:
            ...
            try:
                ...
                err_text = f"Не удалось создать временную папку {temp_dir}: {e}"
    ...
    def _cleanup_temp_dir(
        ...
        **Алгоритм:**
        ...
        # Если временная папка не создавалась, ничего не делаем
        if temp_dir is None:
            return
        ...
        err = self._del_file(
            ...
            force=True, # удаляем полностью со всем содержимым
        )
        # только если ошибки нет (или удаление отложено), удаляем ключ
        if err is None:
    ...
    def _prepare_new_dto(
        self,
        overrides: Optional[Dict[str, Any]] = None,
        assign_temp_id: bool = True
    ) -> Optional[Any]:
        """
        Создаёт новый DTO для новой строки, заполняя все поля DTO (включая скрытые)
        значениями из контекстных параметров (_context_params) и переопределений.

        Args:
            overrides: Словарь с переопределениями полей (например, {'file_path': '...'}).
            assign_temp_id: Если True, присваивает временный отрицательный ID.
                            Если False, ID остаётся None (будет присвоен позже).

        Returns:
            DTO или None, если не удалось создать (ошибка валидации, отсутствие обязательных полей).
        """
        # Создаём словарь для всех полей DTO (включая скрытые)
        ...
        if missing:
            self.logger.error(f"Не удалось создать новую строку: отсутствуют обязательные поля {missing}")
            return None
        ...
        try:
            ...
            self.logger.exception(f"Ошибка создания DTO: {e}")
            return None

        # Присваиваем временный ID, если нужно
        if assign_temp_id:
    ...
    def _get_photo_field_name_impl(self) -> Optional[str]:
        """
        Возвращает имя первого поля с widget_type='image_thumbnail' в field_configs.

        Returns:
            Имя поля или None, если такое поле отсутствует.
        """
        for field_name, config in self.field_configs.items():
            if config.get('widget_type') == 'image_thumbnail':
                return field_name
    ...
    def _has_photo_column(self) -> bool:
        ...
        return self._get_photo_field_name_impl() is not None
    ...
    def _get_current_photo_full_path(
        self,
        dto: Any,
        photo_field: str,
        ...
        """
        Возвращает абсолютный путь к текущему файлу фото (если есть),
        проверяя сначала временную папку черновика, затем основное хранилище.
        """
    ...
    def _show_photo_edit_dialog(
        ...
        temp_dir = self._ensure_temp_dir(entity_id) if entity_id is not None else None
    ...
    def _add_photo_to_row_impl(
        ...
        """
        Реализация добавления фото в строку.

        Args:
            row: Индекс строки в модели (source_model).
            file_path: Абсолютный путь к файлу изображения.
            photo_field: Имя поля с фото (если None, определяется автоматически).
        ...
        if row < 0 or row >= self.source_model.rowCount():
            self.logger.warning(f"_add_photo_to_row_impl: неверный индекс строки {row}")
            return False
        ...
        if photo_field is None:
            ...
            if photo_field is None:
                self.logger.error("_add_photo_to_row_impl: нет поля с widget_type='image_thumbnail'")
                return False

        # Проверка расширения
        ext = os.path.splitext(file_path)[1].lower()
        allowed = self._get_allowed_extensions_for_photo(photo_field)
        if ext not in allowed:
            self.logger.warning(f"_add_photo_to_row_impl: недопустимое расширение {ext}")
            return False

        entity_id = getattr(dto, 'id', None)
        is_new = (entity_id is None or entity_id < 0)

        # Если строка новая – создаём временный ID
        if is_new:
            if entity_id is None:
                ...
                # Регистрируем новую строку в реестре, если ещё нет
                if not self._draft_registry.has(f"__new__:{self._entity_type}:{entity_id}"):
    ...
    def edit_photo_in_row(
        ...
        """
        Реализация редактирования фото в строке.
        ...
        # Если строка новая и нет ID – создаём временный
        if is_new and entity_id is None:
            entity_id = self._next_temp_id
            self._next_temp_id -= 1
            dto.id = entity_id
            self.source_model.update_row(row, dto)
    ...
    def _delete_photo_in_row_impl(
        self,
        row: int,
        photo_field: Optional[str] = None
    ) -> bool:
        """
        Реализация удаления фото из строки.
        ...
        Returns:
            True, если поле было не пустым и очищено, иначе False.
        """
        if row < 0 or row >= self.source_model.rowCount():
            self.logger.warning(f"_delete_photo_in_row_impl: неверный индекс строки {row}")
            return False
        ...
        if photo_field is None:
            photo_field = self._get_photo_field_name_impl()
            if photo_field is None:
                self.logger.error("_delete_photo_in_row_impl: нет поля с widget_type='image_thumbnail'")
                return False

        current = getattr(dto, photo_field, None)
        if current is None:
            return False
        ...
        # Очищаем поле описания, если оно есть
        desc_field = self.field_configs.get(photo_field, {}).get('description_field')
        if desc_field and hasattr(dto, desc_field):
            setattr(dto, desc_field, "")
    ...
    def _clear_page_drafts_prefixes(self) -> None:
        ...
        for key, temp_dir in temp_items:
            ...
            err = self._del_file(
                file_path=temp_dir, # возможны проблемы с типом данных
                # session=None,  # Моментальное удаление
                ctx=None, # Моментальное удаление
                if_delete_parent_dir=False,
                force=True,  # удаляем полностью со всем содержимым
            )
            if err:
        ...
        if err_text:
            self.logger.warning("Не удалось удалить " +" | ".join(err_text))
    ...
    def _convert_photo_paths_to_absolute(
        self,
        entity_id,
        dto,
    ):
        ...
        if temp_dir:
            for field_name, config in self.field_configs.items():
                if config.get('widget_type') != 'image_thumbnail':
                    continue
                rel_path = getattr(dto, field_name, None)
                if rel_path and isinstance(rel_path, str) and not os.path.isabs(rel_path):
    ...
    def _save_new_row_recursive(
        ...
        **Примечания:**
        ...
        with self.service._session_scope(session) as sess:
            ...
            if dto is not None:
                _parent_id = self._get_parent_id_for_new_row(dto) 

                # Дополнительная проверка: если родитель помечен как новая строка (ещё не сохранён в БД)
                if self._draft_registry.has(f"__new__:{self._get_parent_entity_type(_parent_id)}:{_parent_id}"):
                    raise ValueError(
                        f"Родительская запись с ID={_parent_id} ещё не сохранена в БД (помечена как новая). "
                        "Сначала сохраните родителя, затем сохраняйте дочерние изменения."
                    )
        ...
        **Алгоритм:**
        ...
        try:
            ...
            with self.service._session_scope(session) as sess:
                ...
                self._save_child_components_for_parents(
                    filtered_ids,
                    session = sess
                )
                ...
                ctx_rollback = DeletionContext.create(
                    session,
                    # [
    ...
    def _save_child_changes(
        ...
        **Важно:**
        ...
        Returns:
            None
        """
        # ее = self._get_parents_with_child_drafts()
    ...
    def _discard_all_changes(self) -> None:
        ...
        **Примечание:**
        ...
        # Сохраняем ID текущей выбранной строки до перезагрузки
        ...
        # 6. Восстанавливаем выделение, если сохранённый ID существует и строка присутствует в новых данных
    ...
    def _save_all_changes_impl_session(
        ...
        **Примечания:**
        ...
        with self.service._session_scope(session) as sess:
            ...
            self._save_child_changes(new_map, session=sess)

            # Сохраняем изменённые строки
            self._save_modified_rows(session=sess)
        ...
        **Примечание:** 
        ...
        with self.service._session_scope(session) as sess:
            ...
            for entity_id in entity_ids:
                ...
                # # Удаляем временную папку отложенно после коммита
                # self._cleanup_temp_dir(
                #     entity_id,
                #     ctx=ctx_commit
                # )
    ...
    def _save_child_components_for_parent(
        ...
        **Примечание:** В текущей реализации PaginatedListPage этот метод не используется
        ...
        Args:
            parent_id: ID или свои или свои как родительской сущности (должен быть >= 0, то есть существовать в БД).
            session: Сессия SQLAlchemy (опционально, для работы в одной транзакции).
        """

        if parent_id is None or parent_id < 0:
            # # Родитель ещё не сохранён (временный ID) – сохранять дочерние черновики нельзя
            # self.logger.warning(
            #     f"_save_child_components_for_parent: попытка сохранить дочерние черновики для несохранённого родителя (parent_id={parent_id}). "
            #     "Сначала сохраните родительскую строку."
            # )
            # raise ValueError(
            #     f"Нельзя сохранить дочерние черновики для несохранённого родителя (ID={parent_id}). "
            #     "Сначала сохраните родительскую запись."
            # )
            return

        # # Получаем тип родительской сущности (например, для фото это 'appointment')
        # parent_type = self._get_parent_entity_type(parent_id)
        # if parent_type is None:
        #     parent_type = self._entity_type  # fallback

        # # Дополнительная проверка: если родитель помечен как новая строка (ещё не сохранён в БД)
        # if self._draft_registry.has(f"__new__:{parent_type}:{parent_id}"):
        #     raise ValueError(
        #         f"Родительская запись с ID={parent_id} ещё не сохранена в БД (помечена как новая). "
        #         "Сначала сохраните родителя, затем сохраняйте дочерние изменения."
        #     )
        ...
        for child in self._children_components:
            if hasattr(child, 'apply'):
                ...
                if (key is not None) and self._draft_registry.has(key):
                    ...
                    # Уведомляем родительский фрейм об изменении дочерней сущности
                    # self._update_affected_parents_and_deferred_action(parent_id, parent_id, session)
                    ctx = ActionContext(session, ActionType.COMMIT) if session else None
                    add_deferred_action(
                        ctx,
                        self._update_row_color_by_id,
                        args=(parent_id,)
                    )
    ...
    def _get_row_color(self, dto: Any) -> QColor:
        """
        Определяет цвет фона строки на основе статуса сущности в реестре черновиков.

        Цветовая схема:
            - Зелёный (NEW): новая строка (временный ID, присутствует ключ __new__).
            - Красный (DELETED): строка помечена на удаление (ключ __deleted__).
            - Жёлтый (MODIFIED): статус 'own', 'child' или 'both'.
            - Белый (NORMAL): без изменений.

        Args:
            dto: DTO строки (должен содержать атрибут `id`).

        Returns:
            QColor: Цвет фона для строки.
        """
    ...
    def _add_new_row_at_pos(self, dto: Any) -> None:
        """
        Вставляет новую строку в модель таблицы на рассчитанную позицию.

        Позиция определяется на основе текущего выделения (вставка после выделенной строки)
        или первой видимой строки. Выполняет регистрацию черновика, пометку изменений,
        выделение новой строки и обновление UI.

        Args:
            dto (Any): DTO новой строки (уже должен иметь временный отрицательный ID).
    ...
    def get_current_selected_row(self) -> Optional[int]:
        """
        Возвращает индекс текущей выбранной строки в исходной модели (source_model).
        Если строка не выбрана или индекс невалиден, возвращает None.
        """
        current = self.table_view.currentIndex()
        if current.isValid():
            source_index = self.proxy_model.mapToSource(current) if hasattr(self, 'proxy_model') else current
    ...
    def get_photo_field_name(self) -> Optional[str]:
        """
        Возвращает имя первого поля с widget_type='image_thumbnail' в field_configs.
        Используется для проверки наличия фото-столбца и получения его имени.
        """
        return self._get_photo_field_name_impl()
    ...
    def has_photo_column(self) -> bool:
        """Проверяет, есть ли в таблице столбец с фото."""
        return self._has_photo_column()
    ...
    def add_photo_from_file(self, file_path: str, photo_field: Optional[str] = None) -> bool:
        """
        Добавляет новую строку с фото из указанного файла.
        Если photo_field не указан, определяется автоматически.
    ...
    def add_photo_to_row(
        ...
        """
        Добавляет фото в существующую строку (индекс в source_model).
        Если строка новая (временный ID), создаёт временную папку и помечает как изменённую.
    ...
    def delete_photo_in_row(
        self,
        row: int,
        photo_field: Optional[str] = None
    ) -> bool:
        """
        Удаляет фото из указанной строки (очищает поле file_path и поле описания, если есть).
        Возвращает True, если поле было не пустым и очищено.
