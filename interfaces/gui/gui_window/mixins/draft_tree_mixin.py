...
**Синхронизация дочерних компонентов (виджетов):**
        - Каждый дочерний компонент, реализующий `IEditableComponent`, при добавлении
          через `add_draft_child` подписывается на изменения реестра (метод `_subscribe_to_registry`).
        - При удалении или изменении черновика, связанного с этим компонентом, реестр испускает сигнал
          `draft_changed(key, has_draft)`. Компонент получает этот сигнал и вызывает
          `load_from_registry(registry)`, который перезагружает UI из актуального состояния реестра.
        - Поэтому после операций, изменяющих реестр (например, `discard_entity_subtree`), нет
          необходимости вручную вызывать `_load_drafts_for_children()` – дочерние компоненты
          обновляются автоматически благодаря подписке.

**Методы для работы со статусами:**
    - `mark_own_change(entity_id)`: Помечает сущность как имеющую собственные изменения.
      Вызывает `_update_own_change(entity_id, True)`, обновляет статус в реестре и распространяет вверх.
    - `clear_own_change(entity_id)`: Снимает флаг собственных изменений.
      Вызывает `_update_own_change(entity_id, False)`. **Не изменяет счётчик родителей** – это делается отдельно.
    - `mark_child_change(parent_id, delta)`: Уведомляет родителя об изменении количества активных потомков.
      delta = +1 (появился), -1 (исчез). Обновляет счётчик в реестре и пересчитывает статус родителя.
    - `discard_entity_subtree(entity_id)`: Полностью отменяет изменения для сущности и всех её потомков.
      Удаляет все черновики, статусы и счётчики. Уменьшает счётчик родителя (если был).
    - `clear_entity_drafts(entity_id)`: Удаляет **только собственные черновики** сущности,
      оставляя дочерние нетронутыми. Используется после сохранения сущности.
...
class DraftTreeMixin:
    ...
    def add_draft_child(self, child: IHierarchicalEditableComponent) -> None:
        ...
        **Автоматическая подписка:**
            Если дочерний компонент реализует методы `subscribe_to_registry` и
            `get_draft_key`, он подписывается на изменения своего ключа в реестре.
            Это позволяет ему автоматически обновлять UI при удалении/изменении
            черновика извне (например, при отмене изменений родительской строки).

        **Уведомление родителя об изменении количества черновиков:**
            Через `set_draft_change_notifier` компонент получает callback,
            который вызывает `mark_child_change(parent_id, delta)` для обновления
            счётчика потомков у родительской страницы.

        **Примечание:**
            Если ключ дочернего компонента зависит от `parent_id` (например,
            "appointment:{parent_id}:photos"), то после смены родителя необходимо
            вызвать `child.update_parent_id(new_parent_id)`, чтобы компонент
            переподписался на новый ключ. Это должно быть сделано на уровне
            родительской страницы (см. `PaginatedListPage._update_draft_key_for_selected`).
        ...
        if child not in self._children_components:
            ...
            # Передаём callback для уведомления родителя об изменении количества черновиков
            ...
            # ★ НОВОЕ: подписываем дочерний компонент на его ключ в реестре
            if hasattr(child, 'subscribe_to_registry') and self._draft_registry:
    ...
    def _update_child_change(self, parent_id: int, delta: int) -> None:
        ...
        **Алгоритм:**
            ...
            3. Если новое значение счётчика стало отрицательным – принудительно устанавливаем его в 0 и логируем предупреждение (дисбаланс вызовов mark_child_change)
            4. Пересчитываем статус родителя (с учётом его собственных изменений).
        ...
        if new_count < 0:
            self.logger.warning(
                ...
                "Принудительная коррекция: счётчик установлен в 0."
            )
            

            # =================================================================
            # КОРРЕКЦИЯ ОТРИЦАТЕЛЬНОГО СЧЁТЧИКА
            # =================================================================
            # **ПРИЧИНА ВОЗНИКНОВЕНИЯ:**
            #   Счётчик детей (`__counter__`) увеличивается на +1 при каждом появлении активного
            #   черновика у потомка (вызов `mark_child_change(parent_id, +1)`) и уменьшается на -1
            #   при его исчезновении (вызов `mark_child_change(parent_id, -1)`).
            #
            #   Отрицательное значение счётчика может возникнуть, если количество вызовов с delta = -1
            #   превысило количество вызовов с delta = +1. Это указывает на **дисбаланс** в логике
            #   уведомлений, например:
            #     - Потомок создал черновик, но при его удалении не вызвал `mark_child_change(parent_id, -1)`.
            #     - Потомок был удалён из реестра другим способом (например, прямым вызовом `discard`),
            #       минуя уведомление родителя.
            #     - Ошибка в алгоритме, приводящая к лишнему вызову `mark_child_change(parent_id, -1)`
            #       без соответствующего увеличения.
            #
            # **ПОЧЕМУ КОРРЕКЦИЯ БЕЗОПАСНА И НЕОБХОДИМА:**
            #   Счётчик детей НЕ ДОЛЖЕН БЫТЬ ОТРИЦАТЕЛЬНЫМ, поскольку он отражает реальное количество
            #   активных потомков с ненулевым статусом. Отрицательное значение противоречит физическому смыслу
            #   и может привести к неверному вычислению статуса родителя (например, родитель может остаться
            #   в статусе `'child'`, когда на самом деле у него нет изменённых потомков).
            #
            #   Принудительная установка счётчика в 0:
            #     - Гарантирует, что дальнейшие вычисления статуса будут исходить из корректного состояния
            #       (нет активных черновиков у потомков).
            #     - Не нарушает целостность данных, потому что отрицательное значение не имеет реального смысла.
            #     - Позволяет приложению продолжить работу, не блокируя пользователя из-за ошибки в учёте.
            #
            # **ЧТО ДЕЛАТЬ РАЗРАБОТЧИКУ ПРИ ОБНАРУЖЕНИИ ПРЕДУПРЕЖДЕНИЯ:**
            #   Предупреждение в логе (`self.logger.warning`) должно быть тщательно проанализировано.
            #   Необходимо проверить все места, где вызывается `mark_child_change` (или `_update_parent_child_counter`),
            #   и убедиться, что каждый вызов с delta = +1 парный с вызовом delta = -1, и что они не дублируются.
            #   Особое внимание уделить сценариям с сохранением/отменой новых строк и дочерних черновиков.
            # =================================================================

            # Принудительно устанавливаем счётчик в 0
            self._draft_registry.inc_child_counter(
                self._entity_type, parent_id, -new_count  # компенсируем до 0
            )
            new_count = 0
    ...
    def discard_entity_subtree(self, entity_id: int) -> None:
        ...
        **Важное замечание о синхронизации UI:**
            При удалении черновиков из реестра (`discard`) для каждого ключа испускается сигнал
            `draft_changed(key, has_draft=False)`. Любой компонент, подписанный на этот ключ
            (например, дочерний виджет фото), получит уведомление и сможет самостоятельно
            перезагрузить своё состояние через `load_from_registry`. Поэтому вызывающему коду
            **не нужно** вручную обновлять дочерние виджеты после вызова этого метода.
