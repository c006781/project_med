...
Хранит все несохранённые изменения в плоской структуре (ключ -> состояние).
Не является синглтоном, должен создаваться в корневом компоненте приложения
и передаваться по ссылке во все дочерние компоненты.

Сигналы (через Qt) опциональны, можно использовать обычные callbacks.
"""
