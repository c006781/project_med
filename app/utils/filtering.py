...
def apply_filters(
    ...
    Возвращает:
        модифицированный запрос. В запрос добавляется атрибут _post_filters,
        содержащий список условий для пост-обработки (нечеткий поиск).
    """
    conditions = []
    ...
    # Сохраняем пост-фильтры как атрибут запроса
    query._post_filters = post_filters
    return query
