...
class FilterTableView(QTableView):
    ...
    def refreshRow(self, row: int) -> None:
        ...
        **Назначение:**  
            Удобный метод для вызова из делегатов после асинхронной загрузки миниатюр
            или после редактирования многострочного текста. Объединяет `resizeRowToContents`
            и `update_row`.
