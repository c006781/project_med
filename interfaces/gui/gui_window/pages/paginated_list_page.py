...
class PaginatedListPage(
    ...
    **Работа с множественными дочерними компонентами разных типов:**
    ...
    def _cancel_selected_rows_changes(self):
        """
        **Что делает метод:**
            1. Для каждой выбранной строки:
                - Если строка НОВАЯ (id < 0):
                    * Полностью удаляет строку и все её дочерние новые строки (рекурсивно).
                    * Не уведомляет родителя (родитель никогда не получал уведомления о создании).

                - Если строка СУЩЕСТВУЮЩАЯ (id >= 0):
                    * Если строка помечена на удаление (имеет ключ __deleted__):
                        - Снимает пометку на удаление (восстанавливает строку).
                        - **Затем полностью отменяет ВСЕ изменения** в основных полях и дочерних черновиках.
                          Это делается, чтобы вернуть строку к исходному состоянию, которое было ДО любых правок.
                        - После этого строка становится чистой (без изменений и без пометки на удаление).
                    * Если строка не помечена на удаление:
                        - Отменяет все изменения (основные поля и дочерние черновики) через `discard_entity_subtree`.
                        - Перезагружает DTO из БД и обновляет модель.

        **Почему для удалённой строки нужно отменять ВСЕ изменения (а не только пометку на удаление):**
            Пользователь мог сначала отредактировать поля строки (жёлтый цвет), а затем пометить её на удаление (красный цвет).
            Нажав «Отменить изменения», он ожидает вернуть строку к состоянию, предшествующему ЛЮБЫМ его действиям,
            то есть убрать и правки, и пометку. Если бы мы только сняли пометку, строка осталась бы жёлтой (изменённой),
            что противоречит интуиции. Поэтому правильное поведение – полный откат.

        **Итоговое состояние после отмены для существующей строки:**
            - Нет пометки на удаление.
            - Нет собственных изменений полей.
            - Нет дочерних черновиков.
            - Цвет строки становится белым (нормальное состояние).
